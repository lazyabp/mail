...
namespace Lazy.Abp.Mailing
{
    ...
    public class MailingWebHostModule : AbpModule
    {
        public override void PreConfigureServices(ServiceConfigurationContext context)
        {
            context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
            {
                options.AddAssemblyResource(
                    typeof(MailingResource),
                    typeof(MailingDomainSharedModule).Assembly,
                    typeof(MailingApplicationContractsModule).Assembly,
                    typeof(MailingWebHostModule).Assembly
                );
            });
        }
        ...
        private void ConfigureMenu(IConfiguration configuration)
        {
            ...
            {
                options.MenuContributors.Add(new MailingWebHostMenuContributor(configuration));
            });
        }
        ...
        private void ConfigureAutoMapper()
        {
            ...
            {
                options.AddMaps<MailingWebHostModule>();
            });
        }
        ...
        private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
        {
            if (hostingEnvironment.IsDevelopment())
            {
                ...
                {
                    options.FileSets.ReplaceEmbeddedByPhysical<MailingDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}Lazy.Abp.Mailing.Domain", Path.DirectorySeparatorChar)));
            ...
            IWebHostEnvironment hostingEnvironment)
        {
            if (!hostingEnvironment.IsDevelopment())
            {
                ...
                context.Services
                    .AddDataProtection()
                    .PersistKeysToStackExchangeRedis(redis, "Mailing-Protection-Keys");
            }
        }
        ...
        public override void OnApplicationInitialization(ApplicationInitializationContext context)
        {
            ...
            {
                options.SwaggerEndpoint("/swagger/v1/swagger.json", "Mailing API");
            });
